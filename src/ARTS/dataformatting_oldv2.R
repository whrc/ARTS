...
merge_data = function(new_data, edited_file) {
  ...
  if (file.exists(edited_file)) {
    ...
    new_data = new_data |>
      ...
      mutate(
        UID = case_when(
          # no repeated RTS, keep the original UID
          is.na(RepeatRTS) ~ UID,
          # one of the repeated RTS came from the ARTS main file, use the UID of the polygon in the main file
          any(str_split(RepeatRTS, ',')[[1]] %in% str_split(Intersections, ',')[[1]]) ~ str_split(RepeatRTS, ',')[[1]][which(str_split(RepeatRTS, ',')[[1]] %in% str_split(Intersections, ',')[[1]])],
          # all of the repeated RTS are from the new file, use the UID of the features with the earliest `BaseMapDate`
          all(!str_split(RepeatRTS, ',')[[1]] %in% str_split(Intersections, ',')[[1]]) ~ get_earliest_uid(., idx, SelfIntersectionIndices)
        )
      ) |>
