...
def split_with_buffer(df, subset_names, probs, tile_size):
    ...
    if len(subset_names) != len(probs):
        raise ValueError("The length of subset_names must be equal to the length of probs.")
    
    # buffer polygons with information from tile_size
    df_buffer = df
    df_buffer['buffer'] = df.buffer(math.sqrt(tile_size**2 * 2)) # may be able to change this to be more conservative, depending on how tiles are centered on polygons
    ...
    grouped_df = df_buffer[['zone', 'buffer']].dissolve(by = 'zone').explode(ignore_index = True)
    
    # get the count of polygons in each group
    grouped_df['count'] = df_buffer.sjoin(grouped_df).groupby(['index_right'])['ID'].count()
    ...
    grouped_df = grouped_df.sort_values(by = 'count', ascending = False, ignore_index = True)

    # prep variables to keep track of how many polygons are in each subset throughout the subset assignment for loop
    target_n = [round(value*df.shape[0]) for value in probs] # could be off by 1 row due to rounding
    ...
    for idx, row in grouped_df.iterrows():
        ...
        complete_categories = [key for key in target_n if  counts[key] == target_n[key]]
        subset_names = [item for item in subset_names if item not in complete_categories]
        probs = {key: prob for key, prob in probs.items() if key not in complete_categories}
        ...
        if idx < grouped_df.shape[0] and counts[subset] + grouped_df['count'].iloc[idx] > target_n[subset]:
            ...
            subset = random.choices(
                ...
                weights = [prob/sum([prob for key, prob in probs.items() if key != skip_subset]) for key, prob in probs.items() if key != skip_subset]
    ...
    groups_df = df_buffer.sjoin(grouped_df)[['ID', 'Long', 'Lat', 'subset', 'geometry']]
