...
your_file = 'rts_dataset_test_polygons_new.geojson'  # set this
...
if (demo) {
  intersections_output_filepath = paste(
    base_dir,
    'Tutorial/mock_dataset/python_output',
...
At this point, you will need to manually check all polygons with intersections against the polygons in the official RTS data set in your preferred GIS software and save the output to `r paste0(str_split(your_rts_dataset_file, '\\.')[[1]][1], '_overlapping_polygons_edited.geojson')` (press Ctrl+Enter while cursor is in the preceding in-line code chunk to see the actual file name, rather than the code to produce the file name).  When possible/necessary, try to find imagery that matches the date of the intersecting polygons - this may require contacting the lab that did the original delineation.
...
```{r}
merged_data_self_intersect = self_intersection(merged_data)
```
...
```{r}
check_intersection_info(merged_data_self_intersect, your_rts_dataset_file)
```
