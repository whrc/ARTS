...
At this point, you will need to manually check all polygons with intersections against the polygons in the official RTS data set and polygons with self intersections against themselves in your preferred GIS software and save the output to `r paste0(str_split(your_rts_dataset_file, '\\.')[[1]][1], '_overlapping_polygons_edited.geojson')` (press Ctrl+Enter while cursor is in the preceding in-line code chunk to see the actual file name, rather than the code to produce the file name).  When possible/necessary, try to find imagery that matches the date of the intersecting polygons - this may require contacting the lab that did the original delineation. Visual examples of the following rules are stored in './Tutorial/RTS Relationship Guidelines.pdf'

Your job is to inspect each of the previously published polygons listed in the 'Intersections' column compared to the new RTS feature and manually copy and paste the UIDs from the 'Intersections' column into the 'RepeatRTS', 'RepeatNegative', 'StabilizedRTS', 'NewRTS', 'MergedRTS', 'SplitRTS', 'AccidentalOverlap', 'FalseNegative', or 'UnknownRelationship' based on the relationship between the two polygons. Similarly, you need to inspect each of the polygons listed in the 'SelfIntersections' column and copy and paste the UIDs from the 'SelfIntersections' column into the 'RepeatRTS', 'RepeatNegative', 'StabilizedRTS', 'NewRTS', 'MergedRTS', 'SplitRTS', 'AccidentalOverlap', 'FalseNegative', or 'UnknownRelationship' based on the relationship between the two polygons. The instructions are slightly different for negative and positive RTS features in the new data set. For postitive RTS features, use the following rules:
...
For negative bounding boxes (no RTS inside the bounding box), use the following rules:

- Paste the UID into the NewRTS column when there is an RTS feature in the 'Intersections' or 'SelfIntersections' column that is a new RTS feature which formed within the negative bounding box in the current row. The date of the new RTS feature must be more recent than the date on the negative bounding box.

- Paste the UID into the RepeatNegative column when there is a negative bounding box in the 'Intersections' or 'SelfIntersections' column that overlaps the negative bounding box in the current row. 

- Paste the UID into the FalseNegative column when there is an RTS feature in the 'Intersections' or 'SelfIntersections' column that predates the negative bounding box in the current row. 
...
Add the 'RepeatRTS', 'RepeatNegative', 'StabilizedRTS', 'NewRTS', 'MergedRTS', 'SplitRTS', 'AccidentalOverlap', 'FalseNegative', and 'UnknownRelationship' columns that you just edited back into `new_dataset`.
