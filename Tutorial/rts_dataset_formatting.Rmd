...
your_file = 'Banks_Island_slumps_v2_meta4.shp'  # set this
dataset_version = 'v.0.0.12-alpha' # set this to the most recent version
...
#new_fields = c("Notes" = "Notes", "BaseMapID" = "BaseMapID") #Nit, MSI, and Barth have BaseMapID columns
#new_fields_abbreviated = c('Notes', 'BaseMapID')
```
...
new_dataset$Area = as.character(new_dataset$Area)
new_dataset$Notes = as.character(new_dataset$Notes)
...
new_version = str_flatten(c(str_split(dataset_version, '\\.', n = 4)[[1]][-4], new_increment), '.')
postfix = str_split_i(dataset_version, '-', -1)

new_version = str_flatten(c(new_version, postfix), '-')
new_version
